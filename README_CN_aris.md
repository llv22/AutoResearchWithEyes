...
- **2025-03-11** — 🚀 三大工作流端到端贯通：一句 prompt → 三大会风格论文。`/autor.research-pipeline` 自动串联 idea 发现 → 自动 review → 论文写作
- **2025-03-09** — 📝 `/autor.paper-writing` 工作流：叙事报告 → 大纲 → 图表 → LaTeX → PDF → 两轮自动改进（4/10 → 8.5/10）
...
codex auth login
# Codex MCP 从 .mcp.json 自动配置（项目级）

# 3. 在 Claude Code 中使用
claude
> /autor.idea-discovery "你的研究方向"              # 工作流 1：文献调研 → 头脑风暴 → 验证
> /autor.auto-review-loop                          # 工作流 2：审稿 → 修复 → 再审，一夜完成
> /autor.paper-writing "NARRATIVE_REPORT.md"       # 工作流 3：研究叙事 → 精修 PDF
> /autor.research-pipeline "你的研究方向"            # 全流程：工作流 1 → 2 → 3 端到端
```
...
- 📊 **18 个可组合 skill** — 自由混搭，或串联为完整流水线（`/autor.idea-discovery`、`/autor.auto-review-loop`、`/autor.paper-writing`、`/autor.research-pipeline`）
...
- **探索新方向（比如写 survey）？** 从工作流 1 开始 → `/autor.idea-discovery`
- **已有 idea + 初步方案？** 直接用工作流 2 → `/autor.auto-review-loop`
- **准备写论文了？** 工作流 3 → `/autor.paper-writing`（或分步：`/paper-plan` → `/paper-figure` → `/paper-write` → `/paper-compile` → `/auto-paper-improvement-loop`）
- **全流程？** 工作流 1 → 工作流 2 → 工作流 3 → `/autor.research-pipeline`，从文献调研一路到投稿
...
```
/research-lit → /idea-creator → /novelty-check → 实现 → /run-experiment → /autor.auto-review-loop → /paper-plan → /paper-figure → /paper-write → /auto-paper-improvement-loop → 投稿
...
> 💡 **一键调用：** `/autor.idea-discovery "你的研究方向"` 自动跑完整个工作流 1。
...
6. 实现 → /run-experiment → /autor.auto-review-loop    ← 闭环！
```
...
> 💡 **一键调用：** `/autor.auto-review-loop "你的论文主题"` 自动跑完整个工作流 2。
...
用法：
```
> /autor.auto-review-loop 我的 diffusion model 论文
```
...
> **一键调用：** `/autor.paper-writing "NARRATIVE_REPORT.md"` 自动跑完整个工作流 3。
...
| 🔁 [`auto-review-loop`](skills/autor.auto-review-loop/SKILL.md) | 多轮自动 review→修复→再 review 循环（最多 4 轮） | 是 |
...
| 🔭 [`idea-discovery`](skills/autor.idea-discovery/SKILL.md) | **工作流 1 全流程**：research-lit → idea-creator → novelty-check → research-review | 是 |
| 🏗️ [`research-pipeline`](skills/autor.research-pipeline/SKILL.md) | **完整流水线**：工作流 1 → 实现 → 工作流 2 → 工作流 3，从方向到投稿 | 是 |
...
| 📝 [`paper-writing`](skills/autor.paper-writing/SKILL.md) | **工作流 3 全流程**：paper-plan → paper-figure → paper-write → paper-compile → auto-paper-improvement-loop | 是 |
...
2. （仅 review 类 skill 需要）安装 [Codex CLI](https://github.com/openai/codex) 并配置为 MCP server：
   ...
   codex auth login
   ```
   Codex MCP 通过 `.mcp.json`（项目级）自动配置。如需全局可用：
   ```bash
...
| Skill | 事件 | 推送模式 | 交互模式 |
|-------|------|----------|----------|
| `/autor.auto-review-loop` | 每轮出分、循环结束 | 分数 + 结论 | + 等你决定继续/停止 |
...
| `/autor.idea-discovery` | 阶段切换、最终报告 | 各阶段摘要 | + 审批/拒绝 |
| `/autor.research-pipeline` | 阶段切换、流水线结束 | 阶段摘要 | + 审批/拒绝 |
...
行内覆盖：`/autor.idea-discovery "方向" — pilot budget: 4h per idea, 每步等我确认`
...
- [x] **Idea Discovery 流水线** — `/autor.idea-discovery` 一键编排 research-lit → idea-creator → novelty-check → research-review，含 GPU pilot 实验
- [x] **全流程研究管线** — `/autor.research-pipeline` 串联 Workflow 1（idea discovery）→ 实现 → Workflow 2（auto-review-loop），端到端
